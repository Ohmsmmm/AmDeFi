...
const CONFIG_CHAINCODE_NAME = "AmDeFi"
...
class service {
    ...
    async queryWithArg(user ,funcName , args) {
        var functionName = "[Blockchain.query]"
        ...
        return new Promise(async function (resolve, reject) {
            ...
            try {
                ...
                // Submit the specified transaction.
                const argsString = args.map((args) => util.format('%s', args)).join('|');

                logger.debug(`argsString : ${argsString}`)


                // Evaluate the specified transaction.
                const result = await contract.evaluateTransaction(funcName, argsString);
                logger.info(`Transaction has been evaluated, result is: ${result}`);
